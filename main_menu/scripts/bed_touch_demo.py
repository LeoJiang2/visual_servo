...
"""
bed_touch_demo.py
Function: This program is for doing a brief demo of touching a person in bed
...
class Demo:
	...
	def _gripper_zoom(self, length):
		...
		while(length > 0):
			...
			self._extrude(move)
			# #print(move)
			self._position_rigid_arm_ts([self._cur_rigid_arm_config_ts[0], self._cur_rigid_arm_config_ts[1], self._cur_rigid_arm_config_ts[2] + 0.02])
			rospy.sleep(.75)
			length += -step
	...
	def run_demo(self):
		...
		while(run == True):
			...
			self._position_rigid_arm_ts([x_pos, 0.0, scan_height-0.02])
			
			raw_input("Press Enter to continue to next configuration (extrude)")
			self._gripper_zoom(10.0)
			# #self._extrude(15.0)
