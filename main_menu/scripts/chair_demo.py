...
class Demo:
	...
	def run_demo(self):
		...
		while(run == True):
			...
			self._position_rigid_arm([30.0, -90.0, 0.0])
			# #self._position_theta_4(0.0)

			
			raw_input("Press Enter to continue to next configuration ... (soft arm)")
			#self._pan_arm([x_pos, -0.2, scan_height], [x_pos, 0.2, scan_height])
			# #self._zoom(15.0)
			self._position_rigid_arm([30.0, -125.0, -30.0])
			# #self._position_theta_4(0.0)
			
			raw_input("Press Enter to continue to next configuration ... (soft arm)")

			self._position_rigid_arm([0.0, -125.0, -30.0])
			# #self._position_theta_4(0.0)
			...
			self._position_rigid_arm([20.0, 0.0, 90.0, 0.0])
			# #self._position_theta_4(0.0)
