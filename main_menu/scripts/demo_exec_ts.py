...
"""
demo_exec_ts.py
Function: This program is for doing a brief demo of moving the arm around
...
_SOFT_CONFIG_1 = [35.0, 0.0, 0.0]
_SOFT_CONFIG_2 = [15.0, 0.0, 30.0]
_SOFT_CONFIG_3 = [15.0, 25.0, 0.0]
...
class Demo:
	def __init__(self):
		...
		rospy.sleep(4) # Lets the publishers start
	...
	def run_demo(self):
		...
		while(run == True):
			...
			self._position_rigid_arm_ts([0.3, 0.0, 0.6])
			
			raw_input("Press Enter to continue to next configuration 2...")
			self._position_rigid_arm_ts([0.4, 0.0, 0.7])
			
			raw_input("Press Enter to continue to next configuration 3...")
			self._extrude(12)
			
			raw_input("Press Enter to continue to next configuration 4...")
			self._position_soft_arm(_SOFT_CONFIG_1)
			
			raw_input("Press Enter to continue to next configuration 5...")
			self._position_theta_4(-90)
			rospy.sleep(1)
			self._position_soft_arm(_SOFT_CONFIG_2)
			
			raw_input("Press Enter to continue to next configuration (close gripper)...")
			self._gripper_control_pub.publish(1)
			
			raw_input("Press Enter to continue to next configuration (close gripper)...")
			self._gripper_control_pub.publish(0)
			
			raw_input("Press Enter to continue to next configuration 6...")
			self._position_theta_4(45)
			rospy.sleep(1)
			self._position_soft_arm(_SOFT_CONFIG_3)
			rospy.sleep(2)
			self._gripper_control_pub.publish(1)
			rospy.sleep(3)
			self._gripper_control_pub.publish(0)

			raw_input("Press Enter to continue to next configuration 7...")
			self._position_theta_4(0)
			rospy.sleep(1)
			self._position_soft_arm(_SOFT_CONFIG_ZERO)
			rospy.sleep(1)
			self._extrude(0)			
			
	
			raw_input("Press Enter to continue to next configuration 8...")
			self._position_rigid_arm_ts([0.5, 0.0, 0.5])
