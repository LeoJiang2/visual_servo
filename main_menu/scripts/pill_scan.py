...
"""
pill_scan.py
Function: This program is for doing a brief demo of scanning some pills on the table
...
scan_height = 0.4 # 47cm tall table + 5cm for bottle height?
x_pos = 0.4
...
class Demo:
	...
	def run_demo(self):
		...
		while(run == True):
			...
			self._position_rigid_arm_ts([x_pos-0.05, 0.0, scan_height+0.02]) #Approach
			
			raw_input("Press Enter to continue to next configuration 3...")
			self._position_rigid_arm_ts([x_pos, -0.08, scan_height]) #Look at bottle one
						
			raw_input("Press Enter to continue to next configuration (pan to next bottle)")
			self._pan_arm([x_pos, -0.05, scan_height], [x_pos, 0.08, scan_height]) #Look at bottle two
			# #self._position_rigid_arm_ts([x_pos, 0.05, scan_height]) #Look at bottle two
			
			raw_input("Press Enter to continue to next configuration (reposition arm)")
			self._position_rigid_arm_ts([x_pos+0.07, 0.03, scan_height])
			
			# Look Left
			raw_input("Press Enter to continue to next configuration (Look left - Theta 4)")
			self._position_theta_4(90)
			...
			# #self._position_rigid_arm_ts([x_pos+0.04, 0.0, scan_height])
			
			raw_input("Press Enter to continue to next configuration (presurize soft arm)")
			self._position_soft_arm([20.0, 20.0, 0])
			
			raw_input("Press Enter to continue to next configuration (depressurize soft arm)")
			self._position_soft_arm([0.0, 0, 0])
			
			# Look Right
			raw_input("Press Enter to continue to next configuration (Look Right - move arm)")
			self._position_theta_4(0)
			self._position_rigid_arm_ts([x_pos+0.03, 0.0, scan_height+0.04])
			
			raw_input("Press Enter to continue to next configuration (Theta 4)")
			self._position_theta_4(-90)
			
			raw_input("Press Enter to continue to next configuration (presurize soft arm)")
			self._position_soft_arm([20.0, 0.0, 20.0])
			
			raw_input("Press Enter to continue to next configuration (depressurize soft arm)")
			self._position_soft_arm([0.0, 0, 0])
			
			raw_input("Press Enter to continue to next configuration (zero theta 4)")
			self._position_theta_4(0)
			
			raw_input("Press Enter to continue to next configuration (retract)")
			self._extrude(0.0)
			
			raw_input("Press Enter to continue to next configuration (Reposition arm)")
			self._position_rigid_arm_ts([x_pos, 0.0, scan_height])
			...
			# #self._position_rigid_arm_ts([x_pos-0.1, 0.0, scan_height])
