...
"""
bed_demo_scan.py
Function: This program is for doing a brief demo of scanning a person in bed
...
class Demo:
	...
	def run_demo(self):
		...
		while(run == True):
			...
			self._position_rigid_arm_ts([0.4, 0, 0.5])
			
			
			raw_input("Press Enter to continue to next configuration ... (extrusion)")
			self._extrude(10.0)
			
			raw_input("Press Enter to continue to start motion plan ...")			
			self._ramp_soft_arm([20,0,0])
			rospy.sleep(5)
			self._ramp_soft_arm([30,35,0])
			rospy.sleep(5)
			self._ramp_soft_arm([12,0,0])
			rospy.sleep(5)
			self._ramp_soft_arm([25,0,35])
			rospy.sleep(5)
			self._ramp_soft_arm([25,15,0])
			rospy.sleep(5)
			self._ramp_soft_arm([20,0,0])
			rospy.sleep(5)
			self._ramp_soft_arm([20,35,0])
			rospy.sleep(5)
			self._ramp_soft_arm([25,0,0])
			
			raw_input("Press Enter to continue to next configuration ...")	
