...
class Pi_Communications:
	...
	def __init__(self):
		...
		self._joints = [0, 0, 0]
		self._theta_4 = 0
		self._bending_pressure = 0.0
		self._r1_pressure = 0.0
		self._r2_pressure = 0.0
	...
	def _extrusion_callback(self, msg):
		...
		msg.soft_arm_pressure = [self._bending_pressure, self._r1_pressure, self._r2_pressure]
	...
	def _soft_arm_callback(self, msg):
		...
		# Update the state
		self._bending_pressure = msg.bend_press
		self._r1_pressure = msg.rot_1_press
		self._r2_pressure = msg.rot_2_press
		...
		while(not rospy.is_shutdown()):
			while(not self._queue.empty()):
				...
				if(device == "SOFT_ARM"): # Control soft arm
					...
					msg.accumulator_pressure_digital = self._mcp.read_analog_input(_ACCUMULATOR_channel)
